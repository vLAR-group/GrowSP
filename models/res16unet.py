...
class BasicBlockBase(nn.Module):
    ...
    def forward(self, x):
        residual = x
        # shape = ME.SparseTensor(features =torch.ones_like(x.F), coordinate_map_key=x.coordinate_map_key, coordinate_manager = x.coordinate_manager)
        out = self.conv1(x)
        out = self.norm1(out)
        # out = out*shape
        out = self.relu(out)
...
class Res16UNetBase(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        # self.conv0p1s1 = ME.MinkowskiLinear(in_channels, self.inplanes, False)
        ...
        self.final = conv(self.PLANES[7], self.out_channels, kernel_size=1, stride=1, bias=True, D=D)
        self.relu = MinkowskiReLU(inplace=True)
        self.classifier = nn.Linear(self.PLANES[7], self.out_channels)#, bias=False)
    ...
    def forward(self, x, mode='train'):
        ...
        return out.interpolate(x).F#self.final(out)
